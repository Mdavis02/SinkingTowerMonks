...
public class AnimatedItem2D : MonoBehaviour {
	public enum ToggleMode { none, min, max, animStart, animEnd }
	public enum AnimatedItemTrigger { Automatic, ClickKey, Trigger, Impact, NoLocks, MouseToggle }
	public AnimatedItemTrigger actOn = AnimatedItemTrigger.Automatic;
	...
	private float t;
	private CircleCollider2D overlapRange;
	...
	void Start() {
		...
		overlapRange = GetComponent<CircleCollider2D>();
		...
		if(actOn == AnimatedItemTrigger.MouseToggle) {
			...
			else Debug.Log("Error: " + gameObject.name + " AnimatedItem has no solid material to highlight, add a child sprite.");
		}
	}
	public void OnMouseOver() {
		if(rend && actOn == AnimatedItemTrigger.MouseToggle && DialogTree2D.I.currentState < 0) {
			if(Input.GetMouseButtonUp(0)) { Toggle(); rend.material.color = defaultColor; } else rend.material.color = (Input.GetMouseButton(0) ? DialogTree2D.I.colorClick : DialogTree2D.I.colorHighlight);
		}
	}
	...
	void Update() {
        ...
        //if (actOn == AnimatedItemTrigger.NoLocks) {
        //    Vector3 pos3 = transform.position; Vector2 pos = new Vector2(pos3.x, pos3.y);
        //    Debug.DrawRay(pos + overlapRange.offset, Vector3.up * overlapRange.radius);
        //    Debug.DrawRay(pos + overlapRange.offset, Vector3.down * overlapRange.radius);
        //    Debug.DrawRay(pos + overlapRange.offset, Vector3.left * overlapRange.radius);
        //    Debug.DrawRay(pos + overlapRange.offset, Vector3.right * overlapRange.radius);
        //    Debug.DrawLine(pos, new Vector3(0,0), Color.yellow);
        //}
        if (actOn == AnimatedItemTrigger.NoLocks && !animating && overlapRange) {
			//Vector3 pos3 = overlapRange.transform.position; Vector2 pos = new Vector2(pos3.x,pos3.y);
			Vector3 pos3 = transform.position; Vector2 pos = new Vector2(pos3.x,pos3.y);
            Collider2D[] hits = Physics2D.OverlapCircleAll(pos + overlapRange.offset,overlapRange.radius);
			int i = 0;
			for(; i < hits.Length; ++i) {
		...
		} else if(actOn == AnimatedItemTrigger.Automatic ||
				(actOn == AnimatedItemTrigger.ClickKey && (Input.GetMouseButtonDown(0) || (Inventory2D.I.clickKey != KeyCode.None && Input.GetKeyDown(Inventory2D.I.clickKey))))) {
			BeginAction();
		}
	}
	void OnTriggerEnter2D(Collider2D c) { if(!animating && actOn == AnimatedItemTrigger.Trigger) BeginAction(); }
	void OnCollisionEnter2D(Collision2D c) { if(!animating && actOn == AnimatedItemTrigger.Impact) BeginAction(); }
