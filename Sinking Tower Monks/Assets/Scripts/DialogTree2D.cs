...
public class DialogTree2D : MonoBehaviour {
	static public DialogTree2D I = null;
	...
	public void State(int i) {
		if(i >= states.Count) Debug.Log("Error in DialogTree2D script on "+gameObject.name+": state "+i+" is out of range.");
		else currentState = i;
	}
    ...
    public void RedirectOnState(int n) {
        ...
        else Debug.Log("Error in DialogTree2D script on " + gameObject.name + " at state " + currentState + ": turning off redirect on state " + n + ", out of range.");
    }
	...
	void Update() {
		bool click = Input.GetMouseButtonUp(0) || (clickKey != KeyCode.None && Input.GetKeyDown(clickKey));
		if(state != null && click) {
			state.onClick.Invoke();
			if(state.redirectOnClick) { state.redirect = true; if(!state.nextOnClick) currentState = state.redirectState; }
		...
		if(i >= 20) Debug.Log("Error in DialogTree2D script on "+gameObject.name+": infinite redirect loop timed out.");
		if(currentState >= 0) {
