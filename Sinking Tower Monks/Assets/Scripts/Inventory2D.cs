...
public class Inventory2D : MonoBehaviour {
	public class StoredItem { //not serializable, used at runtime
		public Pickup2D script;
		public int count;
		public float y, t;
		public StoredItem(GameObject go) {
			script = go.GetComponent<Pickup2D>();
	...
	static public Pickup2D hoveredStore, currentItem;
	static public Inventory2D I;
	public Color shopHighlight = Color.cyan;
	public Color shopClick = new Color(.5f,1,1);
	public Text visualValue = null;
	public KeyCode clickKey = KeyCode.None;
	public KeyCode cycleBackwardKey = KeyCode.None;
	public KeyCode cycleForwardKey = KeyCode.None;
	...
	void Start() {
		...
		for(int i = 0;i<transform.childCount;++i) {
			...
			if(all.ContainsKey(go.name)) {
				all[go.name].script = go.GetComponent<Pickup2D>();
	...
	void Update() {
		...
		if(list.Count > 0) { //you've collected at least one type of item
			float scroll = Input.GetAxis("Mouse ScrollWheel") + 
				((cycleBackwardKey != KeyCode.None && Input.GetKeyDown(cycleBackwardKey)) ? 1 : 0) +
				((cycleForwardKey != KeyCode.None && Input.GetKeyDown(cycleForwardKey)) ? -1 : 0);
			if(scroll != 0) index = (index + 10*list.Count + (scroll == 0 ? 0 : (scroll < 0 ? 1 : -1))) % list.Count; //cycle the list more safely

			bool stayalive; int i = 0; Pickup2D p, q;
			while(i < list.Count) { //only show the current item selected, but with some fancy motion
				...
				if(i == index && stayalive) { //animate t from 0 (hidden) to 1 (visible)
					list[i].t = Mathf.Min(1,list[i].t + 15*Time.deltaTime);
					p = list[i].script;
					currentItem = p; q = hoveredStore;
					//if(p.canSpend && hoveredStore && Input.GetMouseButtonDown(0)) {
					bool kact = Input.GetMouseButtonDown(0) || (clickKey != KeyCode.None && Input.GetKeyDown(clickKey));
					if(p.canSpend && q && q.isActiveAndEnabled && q.costType == p.name && q.cost <= list[i].count && kact) {
					...
					list[i].t = Mathf.Max(0,list[i].t - 15*Time.deltaTime);
				...
				if(list[i].t > 0) { //active if animating or selected
					go.SetActive(true);
					Vector3 pos = go.transform.localPosition; pos.y = list[i].y + .2f * (Mathf.Sin(.5f*Mathf.PI*list[i].t) - 1);
