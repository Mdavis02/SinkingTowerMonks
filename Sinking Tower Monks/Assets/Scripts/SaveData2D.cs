...
public class SaveData2D : MonoBehaviour {
	...
	public DialogTree2D savedTree = null;
	public Inventory2D savedInventory = null;
	...
	public class SavedDT { //dialog tree state
		...
		public SavedDT(DialogTree2D.TextState dt) { redirect = dt.redirect; restate = dt.redirectState; }
		public void Restore(DialogTree2D.TextState dt) { dt.redirect = redirect; dt.redirectState = restate; }
	}
	...
	public class SavedOb { //object data
		...
		public SavedOb(GameObject go,SaveObject so) {
			...
			AdvancedCharacter2D a; if(so.saveAdvancedCharacterState && (a = go.GetComponent<AdvancedCharacter2D>())) charState = a.stateIndex;
		}
		public void Restore(GameObject go,SaveObject so) {
			...
			AdvancedCharacter2D a; if(so.saveAdvancedCharacterState && (a = go.GetComponent<AdvancedCharacter2D>())) a.stateIndex = charState;
		}
	}
	public void Save() {
		...
		if(savedTree) {
			...
			foreach(DialogTree2D.TextState d in savedTree.states) dlist.Add(new SavedDT(d));
		}
		...
		if(savedInventory) {
			...
			foreach(KeyValuePair<string,Inventory2D.StoredItem> item in Inventory2D.all) items.Add(new SavedIP(item.Key,item.Value.count));
		...
		if(savedInventory) {
			...
			if(items != null) { //inventory was saved
				foreach(SavedIP i in items) { //foreach saved item
					if(Inventory2D.all.ContainsKey(i.name)) Inventory2D.all[i.name].count = i.count; //if all has item, update count
				}
			}
		}
	}
